...
if (length(args) != 3) {
  stop("Usage: Rscript sleuth_analysis_ercc.R <results_dir> <species_name> <reference_dir>")
}

results_dir <- args[1]  # Directory to save sleuth results
species_name <- args[2]  # Species name
reference_dir <- args[3]  # Reference directory

# Set paths for input files
sleuth_metadata_file <- file.path(results_dir, "sleuth_metadata.tsv")
mart_file <- file.path(reference_dir, species_name, "biomart", paste0(species_name, ".mart_export.txt"))
ercc_file <- file.path(reference_dir, species_name, "annotations", "ERCC92_genes.txt")
